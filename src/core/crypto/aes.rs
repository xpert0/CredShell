...
impl Aes256 {
    pub fn new(key: &[u8; 32]) -> Self {
        ...
        for i in 0..8 {
            ...
            round_keys[i] = u32::from_be_bytes(chunk);
        }
    ...
    fn add_round_key(&self, state: &mut [u8; 16], round: usize) {
        let base = round * 4;
        for c in 0..4 {
            let rk = self.round_keys[base + c].to_be_bytes();
            state[c * 4] ^= rk[0];
            state[c * 4 + 1] ^= rk[1];
            state[c * 4 + 2] ^= rk[2];
            state[c * 4 + 3] ^= rk[3];
    ...
    fn mix_columns(&self, state: &mut [u8; 16]) {
        for c in 0..4 {
            ...
            state[c * 4] = x0 ^ (x1 ^ s1) ^ s2 ^ s3;
            state[c * 4 + 1] = s0 ^ x1 ^ (x2 ^ s2) ^ s3;
            state[c * 4 + 2] = s0 ^ s1 ^ x2 ^ (x3 ^ s3);
            state[c * 4 + 3] = (x0 ^ s0) ^ s1 ^ s2 ^ x3;
        }
    }
}
